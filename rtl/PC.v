...
module PC(clk, rst, PCWrite,StallF, NPC, PC,FlushD,StallD,FlushE,ex_PC);
    ...
    output [31:0] ex_PC;    //PC在ID阶段用不到，EXE才需要
    wire [31:0] id_PC;
    ...
    Flopr U_ID_EX_PC (.clk(clk), .rst(rst), .in_data(id_PC), .out_data(ex_PC), .CLR(FlushE), .Stall(1'b0) );
